...
export class AstraApiService {
    ...
    _auth = '57c5dbbbf1fe4d0001000018e4f31816e24e419094fb6fbcafb7be42';
    _url= '/api';
    _body
    _body$ = this._http.get('assets/SoapRequestBody.xml', { responseType: 'text' }).pipe(tap(b => this._body = b), share());


    constructor(
        _http: HttpClient, _indexMapper: MeasurementIndexMapperService, _msgMapper: ErrorMessageMapperService
) {
}

getStaticMeasurements(): Observable<Measurements> {
...
getMeasurements(): Observable<Measurements> {
...
mapToMeasurement(raw: string, indexMapper: MeasurementIndexMapperService, dataValue: (BasicData)=>number): Observable<Measurements>{
    return of(raw).pipe(
